...
nrow(exl_df) # There are 4422 rows of data 
ncol(exl_df) # There are 16 columns 
dim(exl_df) # There are 4422 rows and 16 columns 
colnames(exl_df) # This output lists all 16 of the culumn names.
rownames(exl_df) # This output lists the row names, or the data entered within each row. 
glimpse(exl_df) # This gives you a glimpse at as much data as possible.
str(exl_df) # Provides a compact way to view your data. When would you use this? Is str for "structure?"
summary(exl_df) # Provides a summary of your data file. 
```
...
```{r eval=FALSE, include=FALSE}
select() #retain/drop columns !!Not sure if I am supposed to run something here.
...
exl_df %>%
  group_by(semester) %>%
  summarize(mean_butyrate = mean(butyrate_mM, na.rm = TRUE)) #error --> changed butyrate_mMol_kg to butyrate mM 
```
...
new_df <- exl_df %>%
  group_by(semester, supplement_consumed) %>%
  summarize(mean_butyrate = mean(butyrate_mM, na.rm = TRUE)) #!! changed object?? 
...
exl_df %>%
  group_by(semester, supplement_consumed) %>%
  summarize(mean_butyrate = mean(butyrate_mM, na.rm = TRUE), 
            sd_butyrate = sd(butyrate_mM, na.rm = TRUE), # sd() is standard deviation function
...
exl_df %>%
  group_by(semester, supplement_consumed) %>%
  summarize(mean_butyrate = mean(butyrate_mM, na.rm = TRUE), 
            sd_butyrate = sd(butyrate_mM, na.rm = TRUE), 
            n_butyrate = n()) %>%
  arrange(-mean_butyrate) #drop "-" to put lowest first # still changing butyrate
```
